...
$manager = new bdManager();
	...
	FROM menus INNER JOIN permisos_perfil ON (menus.id_menu = permisos_perfil.id_menu)
	INNER JOIN menus_cat ON menus.`id_menu_cat` = menus_cat.`id_menu_cat` 
	WHERE id_perfil = '".$_GET['idPerfil']."' ORDER BY menu
	...
	$r_menus2 = $manager->ejecutarConsulta("select distinct menus.id_menu as id, menu as text from menus inner join permisos_perfil 
on menus.id_menu = permisos_perfil.id_menu where id_menu_cat = '".$a_menus['id']."' and menus.estatus = 1 and id_perfil = '".$_GET['idPerfil']."' order by menu");
	$j=0;
